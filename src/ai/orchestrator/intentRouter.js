/**
 * Phase 1+2 intent router — rules only.
 * Priority: COMPANY → SERVICES → TEAM → UNSUPPORTED
 *
 * Owner/Founder/Director/established/HQ stay on COMPANY (company.md),
 * so Phase 1 answers remain authoritative.
 */
...
const SERVICE_PATTERNS = [
  ...
  /\b(property\s+management|professional\s+inspection|brokerage|mortgage|after[\s-]?sales|property\s+listing)\b/i,
...
const TEAM_PATTERNS = [
  ...
  // Case-insensitive lead-in; require at least two name tokens
 ...
 * @returns {'COMPANY_INFO'|'SERVICE_INFO'|'TEAM_INFO'|'UNSUPPORTED'}
 */
